...
export async function GET() {
  ...
  // Test 6: World Cup ESPN endpoint
  try {
    const today = new Date().toISOString().split("T")[0].replace(/-/g, "");
    const resp = await fetch(`https://site.api.espn.com/apis/site/v2/sports/soccer/fifa.world/scoreboard?dates=${today}&limit=50`);
    if (resp.ok) {
      const data = await resp.json();
      const events = data.events || [];
      results.worldCupESPN = { status: "OK", gamesFound: events.length, sampleGames: events.slice(0, 2).map(e => e.name || e.shortName || "unknown") };
    } else {
      results.worldCupESPN = { status: "FAIL", error: `HTTP ${resp.status}` };
    }
  } catch (e) {
    results.worldCupESPN = { status: "FAIL", error: e.message };
  }

  // Test 7: World Cup Odds API (soccer_fifa_world_cup)
  if (!oddsKey) {
    results.worldCupOdds = { status: "FAIL", error: "ODDS_API_KEY not set" };
  } else {
    try {
      const resp = await fetch(`https://api.the-odds-api.com/v4/sports/soccer_fifa_world_cup/odds/?apiKey=${oddsKey}&regions=us&markets=h2h&oddsFormat=american`);
      const remaining = resp.headers.get("x-requests-remaining");
      if (resp.ok) {
        const data = await resp.json();
        results.worldCupOdds = { status: "OK", gamesFound: data.length, remaining, sampleGame: data[0] ? `${data[0].away_team} vs ${data[0].home_team}` : "no games yet" };
      } else if (resp.status === 422) {
        results.worldCupOdds = { status: "PLAN_UPGRADE_NEEDED", error: "soccer_fifa_world_cup requires Business tier on Odds API", remaining };
      } else {
        results.worldCupOdds = { status: "FAIL", error: `HTTP ${resp.status}`, remaining };
      }
    } catch (e) {
      results.worldCupOdds = { status: "FAIL", error: e.message };
    }
  }
