...
export async function POST(req) {
  ...
  const results = games.map(g => {
    ...
    // Project scores using Expected Value (rounded xG), NOT matrix mode
    // Matrix mode always returns 0-0 or 1-1 since those are most probable cells
    // Expected Value is more informative and matches our 1X2 probability
    let projHomeGoals = Math.round(xg.homeXG);
    let projAwayGoals = Math.round(xg.awayXG);

    // Sanity check: if rounded score is a draw but one team has clear edge (>8% gap),
    // give +1 to the favorite so projected score matches 1X2 win probability
    if (projHomeGoals === projAwayGoals) {
      const homeEdge = matchOutcomes.homeWin - matchOutcomes.awayWin;
      if (homeEdge > 0.08) projHomeGoals += 1;      // Home team is clear favorite
      else if (homeEdge < -0.08) projAwayGoals += 1; // Away team is clear favorite
      // If neither has 8%+ edge, keep the draw — it's genuinely close
    }
