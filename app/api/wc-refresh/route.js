...
export async function POST(req) {
  ...
  const results = games.map(g => {
    ...
    return {
      ...
      odds,
      consensus: {
        // Standard fields the UI expects
        ...
        homeRank, awayRank,
        // Null out sport-specific fields from other sports
  ...
  return NextResponse.json({
    ...
    sources: {
      oddsAPI: oddsData.error
        ? `x ${oddsData.error}`
        : `OK ${oddsData.games?.length} WC games (${oddsData.remaining} req left)`,
      model: "Bivariate Poisson | FIFA Rankings | 1H xG (41% ratio)"
    }
  });
}
