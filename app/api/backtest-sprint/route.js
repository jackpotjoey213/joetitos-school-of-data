...
const LEAGUE_SCORING_ADJUSTMENT = 1.0;
...
export async function GET(req) {
  ...
  try {
    ...
    for (const g of gamesToProcess) {
      ...
      // Pitcher model total (with calibrations v2)
      const regressedAwayERA = awayERA * ERA_REGRESSION_WEIGHT + LEAGUE_AVG_ERA * (1 - ERA_REGRESSION_WEIGHT);
      const regressedHomeERA = homeERA * ERA_REGRESSION_WEIGHT + LEAGUE_AVG_ERA * (1 - ERA_REGRESSION_WEIGHT);
      const awayStRuns = (regressedAwayERA / 9) * 5.5;
      const homeStRuns = (regressedHomeERA / 9) * 5.5;
      const bpRuns = 1.8;
