...
async function fetchESPNMLB(daysBack = 0) {
  ...
  // Also fetch tomorrow
  const seenIds2 = new Set(events.map(e => e.id));
  const tomorrow = new Date();
  tomorrow.setDate(tomorrow.getDate() + 1);
  const tomorrowStr = tomorrow.toISOString().split("T")[0].replace(/-/g, "");
  try {
    const resp = await fetch(`https://site.api.espn.com/apis/site/v2/sports/baseball/mlb/scoreboard?dates=${tomorrowStr}&limit=50`, { cache: "no-store" });
    if (resp.ok) {
      const data = await resp.json();
      for (const ev of (data.events || [])) { if (!seenIds2.has(ev.id)) { events.push(ev); seenIds2.add(ev.id); } }
    }
  } catch (e) { /* ignore */ }
  return events;
}
