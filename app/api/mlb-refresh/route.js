    ...
    for (const date of (data.dates || [])) {
      for (const g of (date.games || [])) {
        ...
        games.push({
          ...
          awayTeamRaw: awayAbbr,
          homeTeamRaw: homeAbbr,
...
export async function POST(req) {
  ...
  // Match games - try multiple strategies
  const gameSummaries = games.map(g => {
    const odds = parseOddsForGame(oddsData.games, g.away, g.home, g.awayAbbr, g.homeAbbr);
    const gAway = normalizeAbbr(g.awayAbbr);
    const gHome = normalizeAbbr(g.homeAbbr);
    
    // Strategy 1: Both teams match (normalized)
    let mlbGame = mlbData.games?.find(mg => mg.awayTeam === gAway && mg.homeTeam === gHome);
    // Strategy 2: Both teams match (swapped - ESPN might have home/away wrong)
    if (!mlbGame) mlbGame = mlbData.games?.find(mg => mg.awayTeam === gHome && mg.homeTeam === gAway);
    // Strategy 3: Either team matches on normalized abbr
    if (!mlbGame) mlbGame = mlbData.games?.find(mg => (mg.awayTeam === gAway || mg.homeTeam === gHome || mg.homeTeam === gAway || mg.awayTeam === gHome));
    // Strategy 4: Raw abbreviation match
    if (!mlbGame) mlbGame = mlbData.games?.find(mg => (mg.awayTeamRaw === g.awayAbbr || mg.homeTeamRaw === g.homeAbbr));
    // Strategy 5: Team name fuzzy match
    if (!mlbGame) mlbGame = mlbData.games?.find(mg => fuzzyMatch(mg.awayName, g.away) && fuzzyMatch(mg.homeName, g.home));
    if (!mlbGame) mlbGame = mlbData.games?.find(mg => fuzzyMatch(mg.awayName, g.away) || fuzzyMatch(mg.homeName, g.home));
    
    const matchInfo = mlbGame ? `${mlbGame.awayTeam}@${mlbGame.homeTeam} (AP:${mlbGame.awayPitcher?.name||'TBD'} HP:${mlbGame.homePitcher?.name||'TBD'})` : `NO MATCH (tried ${gAway}@${gHome})`;
    return { ...g, liveOdds: odds, mlbInfo: mlbGame || null, matchDebug: matchInfo };
  });
