  ...
  try {
    ...
    for (const rec of (data.records || [])) {
      for (const tr of (rec.teamRecords || [])) {
        ...
        const w = tr.wins || 0, l = tr.losses || 0;
        const gp = w + l || 1;
        const pyth = pythagWinPct(rs, ra);
        const info = { rs, ra, w, l, gp, rpg: +(rs/gp).toFixed(2), rapg: +(ra/gp).toFixed(2), pyth };
        if (abbr) teams[abbr] = info;
        if (name) teams[name] = info;
        // Also store by teamName (e.g. "dodgers")
...
// API-SPORTS: Team batting (hits, walks, total bases, at bats)
async function fetchTeamBatting() {
  ...
  try {
    ...
    for (const t of (data.response || [])) {
      ...
      if (name) teams[name] = {
        rpg: parseFloat(t.games?.runs?.total?.average) || 0,
        hpg: parseFloat(t.games?.hits?.total?.average) || 0,
        hits: t.games?.hits?.total?.total || 0,
        walks: t.games?.bases_on_balls?.total || 0,
        totalBases: t.games?.total_bases?.total || 0,
        atBats: t.games?.at_bats?.total || 0,
        gp: t.games?.played || 1
      };
    }
    return teams;
...
function buildPrediction(g, odds, awayP, homeP, parkFactor, standings, teamBatting) {
  ...
  // --- PITCHER DATA (Improvement #4: 60% recent, 40% season) ---
  const awayERA = parseFloat(awayP?.era) || 4.50;
  const homeERA = parseFloat(homeP?.era) || 4.50;
  const awayWHIP = parseFloat(awayP?.whip) || 1.35;
  const homeWHIP = parseFloat(homeP?.whip) || 1.35;
  // Weighted ERA: 60% last 3 starts, 40% season (flipped from before - recent matters more)
  const awayEffERA = awayP?.recentERA != null ? awayP.recentERA * 0.6 + awayERA * 0.4 : awayERA;
  const homeEffERA = homeP?.recentERA != null ? homeP.recentERA * 0.6 + homeERA * 0.4 : homeERA;
  ...
  // Blend: 50% Log-5, 50% ML-implied (ML incorporates pitcher, Log-5 is team talent)
  const awayWP = hasPyth ? (log5Away * 0.5 + mlAwayWP * 0.5) : mlAwayWP;
  const homeWP = hasPyth ? (log5Home * 0.5 + mlHomeWP * 0.5) : mlHomeWP;
  ...
  // --- RUN PROJECTION ---
  const awayStRuns = (awayEffERA / 9) * 5.5;
  const homeStRuns = (homeEffERA / 9) * 5.5;
  const bpRuns = 1.8;
  const earlySzn = Math.max(1, Math.floor((Date.now() - new Date("2026-03-27").getTime()) / 86400000));
  const earlyFactor = earlySzn <= 14 ? 0.95 : 1.0;

  // Runs each team scores = (what opposing SP allows + bullpen) * park * momentum * early
  const homeScores = Math.round(((awayStRuns + bpRuns * 0.9) * parkFactor * homeMomentum * earlyFactor) * 10) / 10;
  const awayScores = Math.round(((homeStRuns + bpRuns) * awayMomentum * earlyFactor) * 10) / 10;
  ...
  return {
    consensus: {
      ...
      awayPitcher: awayP || null, homePitcher: homeP || null,
