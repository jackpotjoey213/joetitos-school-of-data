...
import { buildMatchupProfile, PARK_FACTORS } from "../../../lib/dataAggregator.js";
import { runPredictions } from "../../../lib/predictionEngine.js";
import { evaluateBets, removeVig } from "../../../lib/betEvaluator.js";

export const maxDuration = 30;

// --- CALIBRATION CONSTANTS ---
const LEAGUE_SCORING_ADJUSTMENT = 0.85;
const LEAGUE_AVG_ERA = 4.10;
const ERA_REGRESSION_WEIGHT = 0.70;
const ELITE_ERA_THRESHOLD = 3.50;
const PITCHING_HARD_CAP = 8.50;
...
function buildPrediction(g, odds, awayP, homeP, parkFactor, standings, teamBatting) {
  ...
  // ERA regression: 70% raw ERA, 30% league average (softer than before)
  // A 2.10 ERA becomes 2.10*0.7 + 4.10*0.3 = 2.70 (keeps most of elite edge)
  // A 7.00 ERA becomes 7.00*0.7 + 4.10*0.3 = 6.13 (still penalized but not as extreme)
  const awayERA = rawAwayERA * ERA_REGRESSION_WEIGHT + LEAGUE_AVG_ERA * (1 - ERA_REGRESSION_WEIGHT);
  const homeERA = rawHomeERA * ERA_REGRESSION_WEIGHT + LEAGUE_AVG_ERA * (1 - ERA_REGRESSION_WEIGHT);
  
  // Weighted ERA: 60% last 3 starts (regressed), 40% season (regressed)
  const awayRecentRegressed = awayP?.recentERA != null ? awayP.recentERA * ERA_REGRESSION_WEIGHT + LEAGUE_AVG_ERA * (1 - ERA_REGRESSION_WEIGHT) : awayERA;
  const homeRecentRegressed = homeP?.recentERA != null ? homeP.recentERA * ERA_REGRESSION_WEIGHT + LEAGUE_AVG_ERA * (1 - ERA_REGRESSION_WEIGHT) : homeERA;
  ...
  // Park factor dampened 50% in projection (1.18 Coors becomes 1.09)
  const dampenedPF = 1.0 + (parkFactor - 1.0) * 0.5;
  const homeScores = ((awayStRuns + bpRuns) * dampenedPF * homeMomentum * homePowerMult * earlyFactor);
  const awayScores = ((homeStRuns + bpRuns) * awayMomentum * awayPowerMult * earlyFactor);
  // Calibration: Apply league scoring adjustment
  let modelTotal = Math.round((homeScores + awayScores) * LEAGUE_SCORING_ADJUSTMENT * 10) / 10;

  // Pitching Hard-Cap: If both starters are elite (rawERA < 3.50), cap total at 8.5
  const bothElite = rawAwayERA < ELITE_ERA_THRESHOLD && rawHomeERA < ELITE_ERA_THRESHOLD;
  const hardCapApplied = bothElite && modelTotal > PITCHING_HARD_CAP;
  if (hardCapApplied) {
    modelTotal = PITCHING_HARD_CAP;
    reasons.unshift(`⚡ Pitching hard-cap: both SPs have ERA < ${ELITE_ERA_THRESHOLD} → total capped at ${PITCHING_HARD_CAP}`);
  }

  // Split runs from modelTotal (MUST stay consistent with O/U call)
  const roundedTotal = Math.round(modelTotal);
  const homeShare = 0.50 + (homeWP - 0.5) * 0.6;
  const rawHome = roundedTotal * homeShare;
  let projHome, projAway;
  if (homeWP >= awayWP) {
    // Home team is favorite: gets ceiling, away gets remainder
    projHome = Math.ceil(rawHome);
    projAway = roundedTotal - projHome;
  } else {
    // Away team is favorite: gets ceiling, home gets remainder
    projHome = Math.floor(rawHome);
    projAway = roundedTotal - projHome;
  }
  if (projAway < 0) { projAway = 0; projHome = roundedTotal; }
  if (projHome < 0) { projHome = 0; projAway = roundedTotal; }

  // --- VOTING ---
  // The displayed total = sum of projected scores. This drives ALL predictions.
  const displayedTotal = projAway + projHome;
  ...
  // Pitcher model (3.0x / 1.5x) — use displayedTotal for consistency
  const pw = hasSP ? 3.0 : 1.5;
  const pitcherEdge = displayedTotal - line;
  if (Math.abs(pitcherEdge) >= 0.3) {
    if (pitcherEdge > 0) { overW += pw; reasons.push(`Pitcher model: ${displayedTotal} > line ${line} (effERA: ${awayEffERA.toFixed(2)}/${homeEffERA.toFixed(2)})`); }
    else { underW += pw; reasons.push(`Pitcher model: ${displayedTotal} < line ${line} (effERA: ${awayEffERA.toFixed(2)}/${homeEffERA.toFixed(2)})`); }
    dataSources.push(`Pitcher Model: ${projAway}+${projHome}=${displayedTotal} projected runs (${pw}x weight)`);
  }
  ...
  // Park (0.75x - reduced 50% from original 1.5x to avoid over-weighting stadiums)
  if (parkFactor >= 1.05) { overW += 0.75; reasons.push(`Park: ${g.homeAbbr} +${((parkFactor-1)*100).toFixed(0)}% hitter-friendly (dampened)`); }
  else if (parkFactor <= 0.96) { underW += 0.75; reasons.push(`Park: ${g.homeAbbr} -${((1-parkFactor)*100).toFixed(0)}% pitcher-friendly (dampened)`); }
  ...
  // STRICT O/U: The displayed score total determines the call. No exceptions.
  const call = displayedTotal > line ? "OVER" : displayedTotal < line ? "UNDER" : "TOSS-UP";
  const edge = displayedTotal - line;
  ...
  // Run the formal betEvaluator for strict edge-filtered value bets
  const matchupForEval = {
    awayAbbr: g.awayAbbr, homeAbbr: g.homeAbbr,
    awayComposite: awayEffERA, homeComposite: homeEffERA,
    awayKBB: +(awayKBB||3).toFixed(2), homeKBB: +(homeKBB||3).toFixed(2),
    awayBlowupRisk: awayKBB < 2.0, homeBlowupRisk: homeKBB < 2.0,
    bothElite: hardCapApplied, homeWP, awayWP
  };
  const liveOddsForEval = odds ? { homeML: odds.homeML, awayML: odds.awayML, totalLine: line, overML: null, underML: null, books: odds.numBooks } : null;
  const { valueBets, mlEdge, ouEdge } = evaluateBets({ homeWP, awayWP, modelTotal, hardCapped: hardCapApplied }, matchupForEval, liveOddsForEval);
  ...
  return {
    consensus: {
      ...
      valueTier, probEdge: probEdge.toFixed(1), hardCapApplied: hardCapApplied || false,
      valueBets, mlEdge, ouEdge,
