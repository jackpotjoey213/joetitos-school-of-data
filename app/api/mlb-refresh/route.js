...
async function fetchPitcherStats(pid) {
  if (!pid) return null;
  try {
    const resp = await fetch(`https://statsapi.mlb.com/api/v1/people/${pid}/stats?stats=season&season=2026&group=pitching`, { cache: "no-store" });
    if (!resp.ok) return null;
    const s = (await resp.json()).stats?.[0]?.splits?.[0]?.stat;
    if (!s) return null;
    return { era: s.era || "0.00", whip: s.whip || "0.00", wins: s.wins || 0, losses: s.losses || 0, strikeOuts: s.strikeOuts || 0, ip: s.inningsPitched || "0.0", hr: s.homeRuns || 0, bb: s.baseOnBalls || 0, gs: s.gamesStarted || 0 };
  } catch { return null; }
}

// --- API-SPORTS: Team batting stats ---
async function fetchTeamStats() {
  const key = process.env.API_SPORTS_KEY;
  if (!key) return { error: "No API-SPORTS key", teams: {} };
  try {
    const resp = await fetch("https://v1.baseball.api-sports.io/teams/statistics?league=1&season=2026", {
      headers: { "x-apisports-key": key }
    });
    if (!resp.ok) return { error: `API-SPORTS ${resp.status}`, teams: {} };
    const data = await resp.json();
    const teams = {};
    for (const t of (data.response || [])) {
      const abbr = t.team?.name || "";
      teams[abbr] = {
        runsPerGame: t.games?.runs?.total?.average || 0,
        hitsPerGame: t.games?.hits?.total?.average || 0,
        battingAvg: t.games?.batting_average || 0,
        wins: t.games?.wins?.total || 0,
        losses: t.games?.loses?.total || 0
      };
    }
    return { teams };
...
export async function POST(req) {
  ...
  const [oddsData, mlbData, teamData] = await Promise.all([
    fetchMLBOdds(),
    fetchMLBPitchers(),
    fetchTeamStats()
  ]);
  ...
  const gameSummaries = games.map(g => {
    ...
    const mlbGame = mlbData.games?.find(mg => fuzzyMatch(mg.awayTeam, g.awayAbbr) || fuzzyMatch(mg.homeTeam, g.homeAbbr) || fuzzyMatch(mg.awayTeam, g.away) || fuzzyMatch(mg.homeTeam, g.home));
  ...
  const results = gameSummaries.map(g => {
    ...
    if ((status === "final" || status === "live") && locked[g.id]) return { id: g.id, status, sport: "mlb", awayScore: g.liveScore?.away ?? null, homeScore: g.liveScore?.home ?? null, clock: g.liveScore?.clock || "", currentTotal: locked[g.id].line || g.total, currentSpread: null, oddsRange: null, numBooks: 0, oddsSpread: null, oddsTotal: null, oddsML: null, consensus: locked[g.id].consensus || null, lockedPrediction: locked[g.id], isNewPrediction: false, injuries: null };
    ...
    const bpRuns = 1.8;
    const homeRuns = Math.round((awayPRuns + bpRuns * 0.9) * parkFactor * 10) / 10;
    const awayRuns = Math.round((homePRuns + bpRuns) * 10) / 10;
    const modelTotal = Math.round((homeRuns + awayRuns) * 10) / 10;

    const awayWP = mlToProb(odds?.awayML);
    const homeWP = mlToProb(odds?.homeML);

    // --- VOTING: Model total ALWAYS drives the primary call ---
    ...
    // MODEL TOTAL vs LINE (always votes - 3.0x with pitchers, 1.5x without)
    ...
    // WHIP signal (1.0x, only with real data)
    if (hasPitchers) {
      const cWHIP = (awayWHIP + homeWHIP) / 2;
      if (cWHIP > 1.40) { overW += 1.0; reasons.push(`WHIP: ${cWHIP.toFixed(2)} (high baserunners)`); }
      else if (cWHIP < 1.05) { underW += 1.0; reasons.push(`WHIP: ${cWHIP.toFixed(2)} (few baserunners)`); }
    }

    // Park factor (1.5x, only if significant)
    ...
    // ML implied run differential (1.0x) - big favorites score more
    if (homeWP > 0.62) { overW += 0.5; reasons.push(`${g.homeAbbr} heavy fav (${(homeWP * 100).toFixed(0)}%) - likely run support`); }
    else if (awayWP > 0.58) { overW += 0.5; reasons.push(`${g.awayAbbr} road fav (${(awayWP * 100).toFixed(0)}%) - likely run support`); }
    ...
    // CONFIDENCE: varies based on edge size, pitcher data quality, signal count
    const votingSigs = reasons.filter(r => !r.startsWith("Odds:") && !r.includes(" SP:")).length;
    const edge = Math.abs(modelEdge);
    let edgeConf = 50;
    if (edge >= 0.3 && edge <= 1.0) edgeConf = 70;
    else if (edge > 1.0 && edge <= 2.0) edgeConf = 80;
    else if (edge > 2.0 && edge <= 3.0) edgeConf = 65; // getting suspicious
    else if (edge > 3.0) edgeConf = 45; // model probably off
    else edgeConf = 35; // no edge
    const pitcherBonus = hasPitchers ? 15 : 0;
    const sigBonus = Math.min(15, votingSigs * 5);
    const strength = Math.min(90, Math.max(25, Math.round(agree * 20 + edgeConf * 0.4 + pitcherBonus + sigBonus)));

    // Moneyline pick
    let mlPick = null, mlReason = "";
    if (hasPitchers) {
      const eraGap = awayERA - homeERA;
      if (eraGap > 0.8) { mlPick = g.homeAbbr; mlReason = `${g.homeAbbr} SP ERA advantage (${Math.abs(eraGap).toFixed(2)}) + home field`; }
      else if (eraGap < -0.8) { mlPick = g.awayAbbr; mlReason = `${g.awayAbbr} SP ERA advantage (${Math.abs(eraGap).toFixed(2)}) on road`; }
    }
    if (!mlPick) {
      if (homeWP > 0.58) { mlPick = g.homeAbbr; mlReason = `${g.homeAbbr} implied ${(homeWP * 100).toFixed(0)}% + home field`; }
    ...
    if (hasPitchers && Math.abs(awayERA - homeERA) > 1.5) {
      ...
      rlPick = better; rlReason = `${better} dominant SP matchup (${Math.abs(awayERA - homeERA).toFixed(2)} ERA gap)`;
    }
  ...
  return NextResponse.json({
    ...
    sources: {
      ...
      apiSports: teamData.error ? `x ${teamData.error}` : `OK ${Object.keys(teamData.teams).length} teams`,
      pitchers: `${Object.keys(pMap).length} games with SP data`
    }
  });
}
