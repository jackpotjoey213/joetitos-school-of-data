...
function buildPrediction(g, odds, awayP, homeP, parkFactor, standings, teamBatting) {
  ...
  const homeScores = ((awayStRuns + bpRuns * 0.9) * parkFactor * homeMomentum * earlyFactor);
  const awayScores = ((homeStRuns + bpRuns) * awayMomentum * earlyFactor);
  // Calibration #1: Apply league scoring adjustment (backtest showed +0.36 OVER bias)
