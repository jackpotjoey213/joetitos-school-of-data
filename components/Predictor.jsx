...
async function fetchLiveUpdates(games, lockedPredictions, sport) {
  const endpoint = sport === "mlb" ? "/api/mlb-refresh" : sport === "nba" ? "/api/nba-refresh" : sport === "worldcup" ? "/api/wc-refresh" : "/api/refresh";
...
function getLockedPredictions(sport) {
  try {
    const key = sport === "mlb" ? "joetito_locked_mlb" : sport === "nba" ? "joetito_locked_nba" : sport === "worldcup" ? "joetito_locked_wc" : "joetito_locked_ncaab";
    ...
    <div style={{ background:"var(--card-bg)",border:`1px solid ${isFinal?"#22c55e30":"var(--border)"}`,borderRadius:"16px",overflow:"hidden",cursor:"pointer" }} onClick={onToggle}>
      <div style={{ padding:"6px 16px",background:"var(--card-header)",display:"flex",justifyContent:"space-between",alignItems:"center",borderBottom:"1px solid var(--border)",fontSize:"11px",color:"var(--muted)",fontWeight:600,letterSpacing:"0.8px",textTransform:"uppercase" }}>
        ...
        <span style={{ display:"flex",alignItems:"center",gap:"6px" }}>
          ...
          {isFinal ? <span style={{color:"#22c55e",fontWeight:700}}>FINAL</span> : isLive ? game.liveScore?.clock||"" : 
            game.startTime ? new Date(game.startTime).toLocaleTimeString("en-US", { hour:"numeric", minute:"2-digit", timeZoneName:"short" }) : game.time}
        </span>
      </div>
          ...
          <div style={{ background:"#10b98110",border:"1px solid #10b98130",borderRadius:"10px",padding:"12px" }}>
            <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"8px" }}>
              <span style={{ fontSize:"10px",fontWeight:700,color:"#10b981",textTransform:"uppercase",letterSpacing:"1px" }}>
                {game.sport === "mlb" ? (liveAdj.consensus.hardCapApplied ? "⚡ Elite Pitching Cap" : "MLB Prediction") : game.sport === "nba" ? "NBA Prediction" : game.sport === "worldcup" ? "⚽ World Cup Prediction" : `KenPom Prediction ${liveAdj.consensus.tournamentRound && liveAdj.consensus.tournamentRound !== "tournament" ? `(${liveAdj.consensus.tournamentRound})` : ""}`}
              </span>
            ...
            {/* Edge-rated prediction badges */}
            {(() => {
              const c = liveAdj.consensus;
              const isNBA = game.sport === "nba";
              const isWC = game.sport === "worldcup";
              const edge = parseFloat(c.edgeSize) || 0;
              const edgeUnit = isNBA ? " pts" : isWC ? " goals" : " runs";
              const line = game.total || liveAdj.currentTotal || 0;
              
              // O/U edge rating
              const ouThreshStrong = isNBA ? 5 : isWC ? 0.4 : 2;
              const ouThreshLean = isNBA ? 2 : isWC ? 0.2 : 0.5;
              const ouRating = edge >= ouThreshStrong ? "strong" : edge >= ouThreshLean ? "lean" : "noedge";
              const ouColors = { strong: { bg:"#22c55e18", color:"#22c55e", border:"#22c55e40", label:"🟢 Strong" }, lean: { bg:"#eab30818", color:"#eab308", border:"#eab30840", label:"🟡 Lean" }, noedge: { bg:"#6b728018", color:"#6b7280", border:"#6b728040", label:"🔴 No edge" } }[ouRating];
              
              // ML edge rating (model WP vs Vegas implied WP)
              const modelFavWP = Math.max(parseFloat(c.awayWinProb)||50, parseFloat(c.homeWinProb)||50);
              const vegasFavWP = c.moneylinePick === game.homeAbbr ? mlToProb(liveAdj.oddsML?.home) : mlToProb(liveAdj.oddsML?.away);
              const mlEdge = Math.abs(modelFavWP/100 - (vegasFavWP||0.5)) * 100;
              const mlRating = mlEdge >= 8 ? "strong" : mlEdge >= 3 ? "lean" : "noedge";
              const mlColors = { strong: { bg:"#22c55e18", color:"#22c55e", border:"#22c55e40", label:"🟢 Strong" }, lean: { bg:"#eab30818", color:"#eab308", border:"#eab30840", label:"🟡 Lean" }, noedge: { bg:"#6b728018", color:"#6b7280", border:"#6b728040", label:"🔴 No edge" } }[mlRating];

              return (
                <div style={{ display:"flex",flexDirection:"column",gap:"6px",marginBottom:"10px" }}>
                  {/* O/U Pick with edge rating */}
                  <div style={{ display:"flex",gap:"6px",justifyContent:"center",alignItems:"center",flexWrap:"wrap" }}>
                    <CallBadge text={`${c.totalCall} ${line > 0 ? line : ""}`} type={c.totalCall?.includes("OVER")?"over":"under"} />
                    <span style={{ background:ouColors.bg, color:ouColors.color, border:`1px solid ${ouColors.border}`, padding:"3px 8px", borderRadius:"6px", fontSize:"9px", fontWeight:700 }}>{ouColors.label} ({edge.toFixed(1)}{edgeUnit})</span>
                  </div>
                  {/* ML Pick with edge rating */}
                  {c.moneylinePick && (
                    <div style={{ display:"flex",gap:"6px",justifyContent:"center",alignItems:"center",flexWrap:"wrap" }}>
                      <span style={{ background:"#6366f118",color:"#6366f1",border:"1px solid #6366f140",padding:"4px 10px",borderRadius:"8px",fontSize:"11px",fontWeight:700 }}>ML: {c.moneylinePick}</span>
                      <span style={{ background:mlColors.bg, color:mlColors.color, border:`1px solid ${mlColors.border}`, padding:"3px 8px", borderRadius:"6px", fontSize:"9px", fontWeight:700 }}>{mlColors.label} ({mlEdge.toFixed(1)}% edge)</span>
                    </div>
                  )}
                  {/* Spread/Pick button — for World Cup show Win, for other sports show "covers" */}
                  {c.spreadCall && (
                    <div style={{ display:"flex",gap:"6px",justifyContent:"center",alignItems:"center" }}>
                      {game.sport === "worldcup" ? (
                        <CallBadge text={`${c.moneylinePick || c.spreadCall} WIN (1X2)`} type="spread" />
                      ) : (
                        <CallBadge text={`${c.spreadCall} covers`} type="spread" />
                      )}
                    </div>
                  )}
                  {/* Large divergence warning */}
                  {edge > (isNBA ? 15 : 5) && (
                    <div style={{ textAlign:"center" }}><span style={{ background:"#ef444418",color:"#ef4444",border:"1px solid #ef444440",padding:"3px 8px",borderRadius:"6px",fontSize:"9px",fontWeight:700 }}>⚠️ Large divergence — check injuries/news</span></div>
                  )}
                </div>
              );
            })()}
        ...
        {isExpanded && liveAdj?.consensus && (
          <div style={{ marginTop:"12px",animation:"fadeIn 0.3s ease" }}>
            ...
            <div style={{ background:"var(--lines-bg)",borderRadius:"10px",padding:"12px",marginBottom:"10px" }}>
              <div style={{ fontSize:"11px",fontWeight:700,color:"#6366f1",textTransform:"uppercase",letterSpacing:"0.8px",marginBottom:"8px" }}>Data Sources Used</div>
              <div style={{ fontSize:"11px",color:"var(--text-secondary)",lineHeight:1.8 }}>
                <div style={{ padding:"3px 0",borderBottom:"1px solid var(--border)" }}><span style={{color:"#22c55e"}}>Odds API</span> - Lines from {liveAdj.numBooks || 0} sportsbooks {liveAdj.oddsRange ? `(range: ${liveAdj.oddsRange.min}-${liveAdj.oddsRange.max})` : ""}</div>
                {game.sport === "mlb" ? (
                  <>
                    <div style={{ padding:"3px 0",borderBottom:"1px solid var(--border)" }}><span style={{color:"#3b82f6"}}>MLB Stats API</span> - Probable pitchers + season stats (ERA, WHIP, W-L)</div>
                    <div style={{ padding:"3px 0",borderBottom:"1px solid var(--border)" }}><span style={{color:"#f59e0b"}}>Park Factors</span> - {liveAdj.consensus.parkFactor ? `${game.homeAbbr} park: ${(liveAdj.consensus.parkFactor * 100).toFixed(0)}%` : "Neutral park"}</div>
                    {liveAdj.consensus.awayPitcher && <div style={{ padding:"3px 0",borderBottom:"1px solid var(--border)" }}><span style={{color:"#ef4444"}}>Away SP</span> - {liveAdj.consensus.awayPitcher.name} ({liveAdj.consensus.awayPitcher.era} ERA, {liveAdj.consensus.awayPitcher.whip} WHIP)</div>}
                    {liveAdj.consensus.homePitcher && <div style={{ padding:"3px 0",borderBottom:"1px solid var(--border)" }}><span style={{color:"#ef4444"}}>Home SP</span> - {liveAdj.consensus.homePitcher.name} ({liveAdj.consensus.homePitcher.era} ERA, {liveAdj.consensus.homePitcher.whip} WHIP)</div>}
                    <div style={{ padding:"3px 0" }}><span style={{color:"#a855f7"}}>ML Implied</span> - {game.awayAbbr}: {liveAdj.consensus.awayWinProb}% / {game.homeAbbr}: {liveAdj.consensus.homeWinProb}%</div>
                  </>
                ) : game.sport === "nba" ? (
                  <>
                    <div style={{ padding:"3px 0",borderBottom:"1px solid var(--border)" }}><span style={{color:"#3b82f6"}}>Pace Model</span> - Projected {liveAdj.consensus.projPace} possessions, model total {liveAdj.consensus.modelTotal}</div>
                    <div style={{ padding:"3px 0",borderBottom:"1px solid var(--border)" }}><span style={{color:"#f59e0b"}}>Defense</span> - {game.awayAbbr} allows {liveAdj.consensus.reasons?.find(r=>r.includes("OPPG"))?.match(/[\d.]+/)?.[0] || "?"} PPG / {game.homeAbbr} allows {liveAdj.consensus.reasons?.filter(r=>r.includes("OPPG"))?.[1]?.match(/[\d.]+/)?.[0] || "?"} PPG</div>
                    {(liveAdj.consensus.awayB2B?.isB2B || liveAdj.consensus.homeB2B?.isB2B) && <div style={{ padding:"3px 0",borderBottom:"1px solid var(--border)" }}><span style={{color:"#ef4444"}}>B2B Alert</span> - {liveAdj.consensus.awayB2B?.isB2B ? game.awayAbbr + " on back-to-back" : ""}{liveAdj.consensus.awayB2B?.isB2B && liveAdj.consensus.homeB2B?.isB2B ? " + " : ""}{liveAdj.consensus.homeB2B?.isB2B ? game.homeAbbr + " on back-to-back" : ""}</div>}
                    <div style={{ padding:"3px 0" }}><span style={{color:"#a855f7"}}>Win Prob</span> - {game.awayAbbr}: {liveAdj.consensus.awayWinProb}% / {game.homeAbbr}: {liveAdj.consensus.homeWinProb}%</div>
                  </>
                ) : game.sport === "worldcup" ? (
                  <>
                    <div style={{ padding:"3px 0",borderBottom:"1px solid var(--border)" }}><span style={{color:"#22c55e"}}>FIFA Rankings</span> - {game.awayAbbr} #{liveAdj.consensus.awayRank} / {game.homeAbbr} #{liveAdj.consensus.homeRank}</div>
                    <div style={{ padding:"3px 0",borderBottom:"1px solid var(--border)" }}><span style={{color:"#3b82f6"}}>Full Match xG</span> - {game.awayAbbr} {liveAdj.consensus.fullMatchXG?.away} / {game.homeAbbr} {liveAdj.consensus.fullMatchXG?.home} → Bivariate Poisson</div>
                    <div style={{ padding:"3px 0",borderBottom:"1px solid var(--border)" }}><span style={{color:"#f59e0b"}}>1H xG (41%)</span> - {game.awayAbbr} {liveAdj.consensus.firstHalfXG?.away} / {game.homeAbbr} {liveAdj.consensus.firstHalfXG?.home}</div>
                    <div style={{ padding:"3px 0",borderBottom:"1px solid var(--border)" }}><span style={{color:"#a855f7"}}>🎯 1H Draw Prob</span> - {liveAdj.consensus.h1DrawProb}% (0-0 + 1-1 + 2-2 diagonal sum)</div>
                    <div style={{ padding:"3px 0",borderBottom:"1px solid var(--border)" }}><span style={{color:"#a855f7"}}>🎯 1H Totals</span> - U0.5: {liveAdj.consensus.h1Totals?.under05}% / O0.5: {liveAdj.consensus.h1Totals?.over05}% | U1.5: {liveAdj.consensus.h1Totals?.under15}% / O1.5: {liveAdj.consensus.h1Totals?.over15}%</div>
                    <div style={{ padding:"3px 0",borderBottom:"1px solid var(--border)" }}><span style={{color:"#ef4444"}}>1X2 Probs</span> - {game.homeAbbr} {liveAdj.consensus.matchOutcomes?.homeWin}% / Draw {liveAdj.consensus.matchOutcomes?.draw}% / {game.awayAbbr} {liveAdj.consensus.matchOutcomes?.awayWin}%</div>
                    {liveAdj.consensus.valueOpportunities?.length > 0 && (
                      <div style={{ padding:"3px 0" }}><span style={{color:"#22c55e"}}>Value Edges</span> - {liveAdj.consensus.valueOpportunities.map(v => `${v.market}: +${v.edge}%`).join(" | ")}</div>
                    )}
                  </>
                ) : (
                  <>
                    <div style={{ padding:"3px 0",borderBottom:"1px solid var(--border)" }}><span style={{color:"#3b82f6"}}>KenPom Model</span> - {game.awayAbbr}: {liveAdj.consensus.awayKenPom?.adjOE || "?"} OE / {liveAdj.consensus.awayKenPom?.adjDE || "?"} DE (#{liveAdj.consensus.awayKenPom?.rank || "?"})</div>
                    <div style={{ padding:"3px 0",borderBottom:"1px solid var(--border)" }}><span style={{color:"#3b82f6"}}>KenPom Model</span> - {game.homeAbbr}: {liveAdj.consensus.homeKenPom?.adjOE || "?"} OE / {liveAdj.consensus.homeKenPom?.adjDE || "?"} DE (#{liveAdj.consensus.homeKenPom?.rank || "?"})</div>
                    <div style={{ padding:"3px 0",borderBottom:"1px solid var(--border)" }}><span style={{color:"#f59e0b"}}>Pace</span> - Projected {liveAdj.consensus.projPace} possessions</div>
                    {liveAdj.consensus.tournamentRound && <div style={{ padding:"3px 0" }}><span style={{color:"#a855f7"}}>Tournament Adj</span> - {liveAdj.consensus.tournamentRound} ({((liveAdj.consensus.tournamentDiscount || 1) * 100).toFixed(0)}% scoring factor)</div>}
                  </>
                )}
              </div>
            </div>
            {/* Edge Analysis */}
            {liveAdj.consensus.edgeSize && parseFloat(liveAdj.consensus.edgeSize) > 0 && (
              <div style={{ background:"var(--lines-bg)",borderRadius:"10px",padding:"12px" }}>
                <div style={{ fontSize:"11px",fontWeight:700,color:"#f59e0b",textTransform:"uppercase",letterSpacing:"0.8px",marginBottom:"6px" }}>Edge Analysis</div>
                <div style={{ fontSize:"12px",color:"var(--text-secondary)",lineHeight:1.5 }}>
                  Model projects {liveAdj.consensus.modelTotal} total vs Vegas line {game.total || liveAdj.currentTotal}. That is a {liveAdj.consensus.edgeSize} point divergence.
                  {parseFloat(liveAdj.consensus.edgeSize) >= 3 && parseFloat(liveAdj.consensus.edgeSize) <= 8 ? " This is a meaningful edge." : ""}
                  {parseFloat(liveAdj.consensus.edgeSize) > 8 ? " This is a large divergence - model may be wrong. Proceed with caution." : ""}
                  {parseFloat(liveAdj.consensus.edgeSize) < 2 ? " Edge is small - close to a coin flip." : ""}
                </div>
              </div>
            )}
          </div>
...
function RecordTracker({ history, weights, sport }) {
  const accentColors = { mlb:"#6366f1", worldcup:"#22c55e", nba:"#f59e0b", ncaab:"#ef4444" };
  const accent = accentColors[sport] || "#6366f1";
  const sportLabel = { mlb:"MLB", worldcup:"⚽ World Cup", nba:"NBA", ncaab:"NCAAB" }[sport] || sport?.toUpperCase();
  const ouLabel = sport === "worldcup" ? "O/U 2.5" : "O/U";
  const mlLabel = sport === "worldcup" ? "1X2" : sport === "ncaab" ? "ATS" : "ML Pick";
  const col3Label = sport === "worldcup" ? "1H Draw" : "Avg Error";
  const col4Label = "Streak";

  if (!history.length) return (
    <div style={{ margin:"0 12px 10px",padding:"12px 14px",borderRadius:"12px",background:"var(--card-bg)",border:`1px solid ${accent}40` }}>
      <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"8px" }}>
        <span style={{ fontSize:"11px",fontWeight:700,color:accent,textTransform:"uppercase",letterSpacing:"1px" }}>{sportLabel} Track Record</span>
        <span style={{ fontSize:"10px",color:"var(--muted)" }}>0 games tracked</span>
      </div>
      <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr",gap:"6px",marginBottom:"8px" }}>
        {[ouLabel, mlLabel, col3Label, col4Label].map((l,i)=>(
          <div key={i} style={{ textAlign:"center",padding:"8px 4px",borderRadius:"8px",background:"var(--lines-bg)" }}>
            <div style={{ fontSize:"16px",fontWeight:800,color:"var(--muted)" }}>—</div>
            <div style={{ fontSize:"8px",color:"var(--muted)",textTransform:"uppercase",fontWeight:600,letterSpacing:"0.3px",marginTop:"3px" }}>{l}</div>
          </div>
        ))}
      </div>
      <div style={{ textAlign:"center",fontSize:"11px",color:"var(--muted)" }}>
        Hit <strong style={{color:accent}}>Refresh</strong> before games start to begin tracking
      </div>
    </div>
  );

  const n = history.length;
  const avgE = (history.reduce((a,h)=>a+Math.abs(h.totalError||0),0)/n).toFixed(1);

  // Streak calculation (based on totalCorrect)
  let streak=0, st="";
  for(let i=n-1;i>=0;i--){
    const w = history[i].totalCorrect;
    if(i===n-1){st=w?"W":"L";streak=1;}
    else if((w&&st==="W")||(!w&&st==="L"))streak++;
    else break;
  }

  // Sport-specific stat calculations
  const ouCorrect = history.filter(h=>h.totalCorrect).length;
  const ouPct = ((ouCorrect/n)*100).toFixed(1);

  const mlTracked = history.filter(h=>h.moneylinePick).length;
  const mlCorrect = history.filter(h=>h.moneylineCorrect).length;
  const mlPct = mlTracked>0 ? ((mlCorrect/mlTracked)*100).toFixed(1) : null;

  const atsCorrect = history.filter(h=>h.spreadCorrect).length;
  const atsPct = ((atsCorrect/n)*100).toFixed(1);

  // World Cup specific
  const h1DrawTracked = history.filter(h=>h.h1DrawPick!=null).length;
  const h1DrawCorrect = history.filter(h=>h.h1DrawCorrect).length;
  const h1DrawPct = h1DrawTracked>0 ? ((h1DrawCorrect/h1DrawTracked)*100).toFixed(1) : null;

  const errorUnit = sport==="mlb" ? "runs" : sport==="worldcup" ? "goals" : "pts";

  // Build stat cards based on sport
  let statCards = [];
  if (sport === "worldcup") {
    const ouLabel = "O/U 2.5";
    statCards = [
      { l:"1X2 Pick", v: mlTracked>0 ? `${mlCorrect}-${mlTracked-mlCorrect}` : "0-0", s: mlPct ? `${mlPct}%` : "—", c: +mlPct>=50?"#22c55e":"#ef4444" },
      { l:ouLabel, v:`${ouCorrect}-${n-ouCorrect}`, s:`${ouPct}%`, c:+ouPct>=55?"#22c55e":+ouPct>=50?"#eab308":"#ef4444" },
      { l:"1H Draw", v: h1DrawTracked>0 ? `${h1DrawCorrect}-${h1DrawTracked-h1DrawCorrect}` : "—", s: h1DrawPct ? `${h1DrawPct}%` : "—", c: h1DrawPct&&+h1DrawPct>=55?"#22c55e":"#eab308" },
      { l:"Streak", v:`${streak}${st}`, s:st==="W"?"🔥":"❄️", c:st==="W"?"#22c55e":"#ef4444" },
    ];
  } else if (sport === "mlb") {
    statCards = [
      { l:"O/U Record", v:`${ouCorrect}-${n-ouCorrect}`, s:`${ouPct}%`, c:+ouPct>=55?"#22c55e":+ouPct>=50?"#eab308":"#ef4444" },
      { l:"ML Record", v: mlTracked>0 ? `${mlCorrect}-${mlTracked-mlCorrect}` : "0-0", s: mlPct ? `${mlPct}%` : "—", c: +mlPct>=55?"#22c55e":+mlPct>=50?"#eab308":"#ef4444" },
      { l:"Avg Error", v:avgE, s:errorUnit, c:+avgE<3?"#22c55e":+avgE<5?"#eab308":"#ef4444" },
      { l:"Streak", v:`${streak}${st}`, s:st==="W"?"🔥":"❄️", c:st==="W"?"#22c55e":"#ef4444" },
    ];
  } else if (sport === "nba") {
    statCards = [
      { l:"O/U Record", v:`${ouCorrect}-${n-ouCorrect}`, s:`${ouPct}%`, c:+ouPct>=55?"#22c55e":+ouPct>=50?"#eab308":"#ef4444" },
      { l:"ML Record", v: mlTracked>0 ? `${mlCorrect}-${mlTracked-mlCorrect}` : "0-0", s: mlPct ? `${mlPct}%` : "—", c: +mlPct>=55?"#22c55e":+mlPct>=50?"#eab308":"#ef4444" },
      { l:"Avg Error", v:avgE, s:errorUnit, c:+avgE<8?"#22c55e":+avgE<14?"#eab308":"#ef4444" },
      { l:"Streak", v:`${streak}${st}`, s:st==="W"?"🔥":"❄️", c:st==="W"?"#22c55e":"#ef4444" },
    ];
  } else {
    // NCAAB
    statCards = [
      { l:"O/U Record", v:`${ouCorrect}-${n-ouCorrect}`, s:`${ouPct}%`, c:+ouPct>=55?"#22c55e":+ouPct>=50?"#eab308":"#ef4444" },
      { l:"ATS Record", v:`${atsCorrect}-${n-atsCorrect}`, s:`${atsPct}%`, c:+atsPct>=55?"#22c55e":+atsPct>=50?"#eab308":"#ef4444" },
      { l:"Avg Error", v:avgE, s:errorUnit, c:+avgE<6?"#22c55e":+avgE<10?"#eab308":"#ef4444" },
      { l:"Streak", v:`${streak}${st}`, s:st==="W"?"🔥":"❄️", c:st==="W"?"#22c55e":"#ef4444" },
    ];
  }
  ...
  // Bias label
  let biasText = null;
  if (sport==="mlb" || sport==="nba" || sport==="worldcup") {
    const totalBias = history.reduce((a,h)=>a+(h.totalError||0),0)/n;
    const direction = totalBias > 0.2 ? "runs high" : totalBias < -0.2 ? "runs low" : "well calibrated";
    biasText = `${totalBias>0?"+":""}${totalBias.toFixed(1)} ${errorUnit} avg (${direction})`;
  }
  ...
  return (
    <div style={{ margin:"0 12px 10px",padding:"14px",borderRadius:"12px",background:"var(--card-bg)",border:"1px solid var(--border)" }}>
      ...
      {/* Streak dots — last 30 results */}
      <div style={{ display:"flex",gap:"3px",flexWrap:"wrap",justifyContent:"center",marginBottom:"10px" }}>
        {history.slice(-30).map((h,i)=>(
          <span key={i}
            title={`${h.away||""}@${h.home||""}: ${h.totalCorrect?"✓":"✗"}`}
            style={{ width:14,height:14,borderRadius:"3px",display:"inline-flex",alignItems:"center",justifyContent:"center",
              background:h.totalCorrect?"#22c55e25":"#ef444425",
              color:h.totalCorrect?"#22c55e":"#ef4444",fontSize:"8px",fontWeight:800 }}>
            {h.totalCorrect?"✓":"✗"}
          </span>
        ))}
      </div>
      ...
      {recent.length > 0 && (
        <div style={{ borderTop:"1px solid var(--border)",paddingTop:"8px",marginBottom:"8px" }}>
          {recent.map((h,i)=>(
            <div key={i} style={{ display:"flex",justifyContent:"space-between",alignItems:"center",padding:"4px 0",borderBottom: i<recent.length-1 ? "1px solid var(--border)" : "none" }}>
              <div>
                <div style={{ fontSize:"11px",color:"var(--text-secondary)" }}>{h.away}@{h.home}</div>
                <div style={{ fontSize:"10px",color:"#a855f7",marginTop:"1px" }}>
                  {h.totalCall} {h.line}
                  {h.moneylinePick ? ` · ${h.moneylinePick} ML` : ""}
                  {h.h1DrawPick ? " · 1H Draw" : ""}
                </div>
              </div>
              <div style={{ textAlign:"right" }}>
                <div style={{ fontSize:"11px",fontWeight:700,color:h.totalCorrect?"#22c55e":"#ef4444" }}>{h.totalCorrect?"WIN":"LOSS"}</div>
                <div style={{ fontSize:"9px",color:"var(--muted)" }}>{h.actualTotal} actual</div>
              </div>
            </div>
          ))}
        </div>
      ...
      {/* Bias row */}
      {biasText && (
        <div style={{ background:"var(--lines-bg)",borderRadius:"6px",padding:"5px 8px",display:"flex",justifyContent:"space-between",alignItems:"center" }}>
          <span style={{ fontSize:"10px",color:"var(--muted)" }}>Model bias</span>
          <span style={{ fontSize:"10px",fontWeight:700,color:"#a855f7" }}>{biasText}</span>
        </div>
      )}
      {sport==="ncaab" && weights.totalBias !== 0 && (
        <div style={{ marginTop:"6px",textAlign:"center",fontSize:"10px",color:"#a855f7" }}>
          🧠 Learned: {weights.totalBias>0?"+":""}{weights.totalBias.toFixed(2)} pts | Tourney: {weights.tourneyFactor.toFixed(3)} | Elite D: {weights.defAdjElite.toFixed(2)}
        </div>
      )}
    </div>
  );
...
export default function App() {
  const [sport, setSport] = useState("worldcup");
  ...
  useEffect(()=>{
    ...
    (async()=>{
    try {
      const endpoint = sport === "mlb" ? "/api/mlb-scores?days=1" : sport === "nba" ? "/api/nba-scores?days=1" : sport === "worldcup" ? "/api/wc-scores" : "/api/scores?days=2";
  ...
  // Track which sports have been auto-refreshed this session
  const refreshedSports = useRef(new Set());
  
  useEffect(() => {
    if (dataLoaded && games.length > 0 && !refreshedSports.current.has(sport)) {
      refreshedSports.current.add(sport);
  ...
  const handleRefresh = useCallback(async()=>{
    ...
    try {
      ...
      if(updates&&Array.isArray(updates)){
        ...
        const ug=games.map(g=>{
          ...
          // LOCK new predictions - use espnId as stable key (sequential id shifts between refreshes)
          const lockKey = g.espnId || g.id;
          if (u.isNewPrediction && u.consensus) {
            const lineToLock = u.currentTotal || ng.total;
            if (lineToLock > 0) {
              lockPrediction(lockKey, {
                totalCall: u.consensus.totalCall,
                strength: u.consensus.strength,
                spreadCall: u.consensus.spreadCall,
                moneylinePick: u.consensus.moneylinePick || null,
                predAway: u.consensus.awayPts || 0,
                predHome: u.consensus.homePts || 0,
                line: lineToLock,
          ...
          // If game is final, record result using LOCKED prediction
          if(u.status==="final"&&u.awayScore!=null&&u.homeScore!=null){
            const lockedPred = getLockedPredictions(sport)[lockKey];
            if(lockedPred && lockedPred.line > 0 && !history.find(h=>h.gameId===lockKey)){
              ...
              const actualWinner = u.awayScore > u.homeScore ? g.awayAbbr : u.homeScore > u.awayScore ? g.homeAbbr : "draw";
              const mlOk = lockedPred.moneylinePick ? lockedPred.moneylinePick === actualWinner : false;
              const spreadOk = lockedPred.spreadCall ? lockedPred.spreadCall === actualWinner : false;

              // World Cup: track 1H Draw separately (0-0 halftime = 1H draw hit)
              // We approximate: if final score is low (0-0 or 1-0) the 1H was likely a draw
              // In a real deployment, halftime scores would be tracked separately
              let h1DrawPick = null, h1DrawCorrect = false;
              if (sport === "worldcup" && lockedPred.consensus?.h1DrawProb > 30) {
                h1DrawPick = "draw";
                // 1H draw = final score suggests low-scoring game (crude approximation)
                h1DrawCorrect = at <= 1;
              }

              const rec = {
                gameId: lockKey, sport, away: g.awayAbbr, home: g.homeAbbr,
                predictedTotal: predTotal || 0, actualTotal: at,
                totalCall: lockedPred.totalCall, line: lockedPred.line,
                totalCorrect: tOk, spreadCorrect: spreadOk,
                moneylineCorrect: mlOk, moneylinePick: lockedPred.moneylinePick || null,
                h1DrawPick, h1DrawCorrect,
                totalError: at - (predTotal || at),
    ...
    setRefreshing(false);
  },[games,weights,history,teamCal,sport]);
    ...
    <div style={{"--bg":"#0a0a0f","--card-bg":"#12121a","--card-header":"#0e0e16","--border":"#1e1e2e","--text":"#e8e8f0","--text-secondary":"#a0a0b8","--muted":"#5a5a78","--accent":"#6366f1","--pred-bg":"#6366f108","--pred-border":"#6366f120","--lines-bg":"#0a0a12","--pill-bg":"#1a1a28","--pill-active":"#6366f1",minHeight:"100vh",background:"var(--bg)",fontFamily:"'Inter',-apple-system,sans-serif",color:"var(--text)",maxWidth:"520px",margin:"0 auto",padding:"0"}}>
      ...
      <div style={{ padding:"28px 20px 20px",background:"linear-gradient(180deg,#12121f 0%,#0a0a0f 100%)",borderBottom:"1px solid var(--border)" }}>
        ...
        <div style={{ display:"flex",alignItems:"center",gap:"8px",marginBottom:"16px" }}>
          ...
          <span style={{ fontSize:"11px",color:"var(--muted)" }}>{sport === "mlb" ? "Baseball" : sport === "nba" ? "Basketball" : sport === "worldcup" ? "FIFA World Cup 2026" : "College Basketball"}</span>
      ...
      {!loading && (
      <>
      <div style={{ marginTop:"12px" }}><RecordTracker history={history.filter(h => (h.sport || "ncaab") === sport)} weights={weights} sport={sport} /></div>

      {/* Model Info */}
      <div style={{ margin:"0 12px",padding:"12px 14px",borderRadius:"10px",background:"var(--pred-bg)",border:"1px solid var(--pred-border)" }}>
        <div style={{ fontSize:"10px",fontWeight:700,color:"var(--accent)",textTransform:"uppercase",letterSpacing:"1px",marginBottom:"6px" }}>{sport === "mlb" ? "MLB Prediction Engine" : sport === "nba" ? "NBA Prediction Engine" : sport === "worldcup" ? "⚽ World Cup Prediction Engine" : "Consensus Model v4"}</div>
        <div style={{ fontSize:"11px",color:"var(--text-secondary)",lineHeight:1.6 }}>
          {sport === "mlb" ? (
            <><strong style={{color:"var(--text)"}}>5 data sources</strong> - Odds API, ESPN, MLB Stats API (pitchers + game logs), API-SPORTS (team batting + ISO), Park Factors. Sabermetric engine with Pythagorean Win%, Log-5 probability, and Runs Created.</>
          ) : sport === "nba" ? (
            <><strong style={{color:"var(--text)"}}>3 data sources</strong> - Odds API (lines from 15+ sportsbooks), ESPN (team stats, PPG, OPPG, schedules), B2B detection. Possession-based model with pace projection and defensive matchup analysis.</>
          ) : sport === "worldcup" ? (
            <><strong style={{color:"var(--text)"}}>2 data sources</strong> - Odds API (h2h + totals, soccer_fifa_world_cup) and ESPN (live scores). Bivariate Poisson model using FIFA Rankings → xG projection. Specializes in <strong style={{color:"var(--accent)"}}>1H Draw</strong> and <strong style={{color:"var(--accent)"}}>1H Totals</strong> derivative markets.</>
          ) : (
      ...
      <div style={{ padding:"12px",display:"flex",flexDirection:"column",gap:"10px" }}>
        {fg.map((g, idx) => {
          const prevDate = idx > 0 ? fg[idx-1].gameDate : null;
          const showDateHeader = g.gameDate && g.gameDate !== prevDate;
          return (
            <div key={g.id}>
              {showDateHeader && (
                <div style={{ padding:"8px 12px",marginBottom:"6px",background:"linear-gradient(90deg,#6366f120,transparent)",borderLeft:"3px solid #6366f1",borderRadius:"0 6px 6px 0",fontSize:"12px",fontWeight:700,color:"#6366f1",letterSpacing:"0.5px" }}>
                  {g.gameDate}
                </div>
              )}
              <GameCard game={g} isExpanded={expandedId===g.id} onToggle={()=>setExpandedId(expandedId===g.id?null:g.id)} weights={weights} liveAdj={liveAdj[g.id]} teamCal={teamCal} />
            </div>
          );
        })}
        {!fg.length&&<div style={{ textAlign:"center",padding:"40px 20px",color:"var(--muted)" }}><div style={{ fontSize:"24px",marginBottom:"8px" }}>{sport==="mlb" ? "\u26BE" : "\uD83C\uDFC0"}</div><div style={{ fontSize:"13px",fontWeight:600 }}>No {sport.toUpperCase()} games match this filter</div></div>}
      </div>
      </>
      )}
