...
export default function App() {
  ...
  useEffect(()=>{(async()=>{
    try {
      ...
      // Database is source of truth — merge with pre-seeded, prefer DB
      if (userData.history.length > 0) {
        const dbIds = new Set(userData.history.map(h => h.gameId));
        const merged = [...userData.history, ...history.filter(h => !dbIds.has(h.gameId))];
        setHistory(merged);
      }
  ...
  useEffect(() => {
    if (dataLoaded && !autoRefreshed.current) {
