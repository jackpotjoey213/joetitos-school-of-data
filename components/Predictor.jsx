...
async function fetchLiveUpdates(games, lockedPredictions, sport) {
  const endpoint = sport === "mlb" ? "/api/mlb-refresh" : sport === "nba" ? "/api/nba-refresh" : "/api/refresh";
...
function getLockedPredictions(sport) {
  try {
    const key = sport === "mlb" ? "joetito_locked_mlb" : sport === "nba" ? "joetito_locked_nba" : "joetito_locked_ncaab";
          ...
          <div style={{ background:"#10b98110",border:"1px solid #10b98130",borderRadius:"10px",padding:"12px" }}>
            <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"8px" }}>
              <span style={{ fontSize:"10px",fontWeight:700,color:"#10b981",textTransform:"uppercase",letterSpacing:"1px" }}>
                {game.sport === "mlb" ? (liveAdj.consensus.hardCapApplied ? "⚡ Elite Pitching Cap" : "MLB Prediction") : game.sport === "nba" ? "NBA Prediction" : `KenPom Prediction ${liveAdj.consensus.tournamentRound && liveAdj.consensus.tournamentRound !== "tournament" ? `(${liveAdj.consensus.tournamentRound})` : ""}`}
              </span>
  ...
  return (
    <div style={{ margin:"0 12px 10px",padding:"14px",borderRadius:"12px",background:"var(--card-bg)",border:"1px solid var(--border)" }}>
      <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"10px" }}>
        <span style={{ fontSize:"11px",fontWeight:700,color:"var(--accent)",textTransform:"uppercase",letterSpacing:"1px" }}>{isMLB ? "MLB" : "NCAAB"} Track Record</span>
      ...
      <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr",gap:"8px" }}>
        {[
          ...
          isMLB ? {l:"ML Record",v:mlTracked>0?`${mc}-${mlTracked-mc}`:"0-0",s:`${mPct}%`,c:+mPct>=55?"#22c55e":+mPct>=50?"#eab308":"#ef4444"} : {l:"ATS Record",v:`${sc}-${n-sc}`,s:`${sPct}%`,c:+sPct>=55?"#22c55e":+sPct>=50?"#eab308":"#ef4444"},
          {l:"Avg Error",v:avgE,s:isMLB?"runs":"pts",c:+avgE<(isMLB?3:6)?"#22c55e":+avgE<(isMLB?5:10)?"#eab308":"#ef4444"},
...
export default function App() {
  ...
  useEffect(()=>{
    ...
    (async()=>{
    try {
      const endpoint = sport === "mlb" ? "/api/mlb-scores?days=1" : sport === "nba" ? "/api/nba-scores?days=1" : "/api/scores?days=2";
        ...
        const ug=games.map(g=>{
          ...
          if(u.status==="final"&&u.awayScore!=null&&u.homeScore!=null){
            ...
            if(lockedPred && lockedPred.line > 0 && !history.find(h=>h.gameId===lockKey)){
              ...
              // Track ML and spread results too
              const actualWinner = u.awayScore > u.homeScore ? g.awayAbbr : g.homeAbbr;
              ...
              const rec={gameId:lockKey,sport:sport,away:g.awayAbbr,home:g.homeAbbr,predictedTotal:predTotal||0,actualTotal:at,totalCall:lockedPred.totalCall,line:lockedPred.line,totalCorrect:tOk,spreadCorrect:spreadOk,moneylineCorrect:mlOk,moneylinePick:lockedPred.moneylinePick||null,totalError:at-(predTotal||at),date:new Date().toISOString().split("T")[0]};
    ...
    <div style={{"--bg":"#0a0a0f","--card-bg":"#12121a","--card-header":"#0e0e16","--border":"#1e1e2e","--text":"#e8e8f0","--text-secondary":"#a0a0b8","--muted":"#5a5a78","--accent":"#6366f1","--pred-bg":"#6366f108","--pred-border":"#6366f120","--lines-bg":"#0a0a12","--pill-bg":"#1a1a28","--pill-active":"#6366f1",minHeight:"100vh",background:"var(--bg)",fontFamily:"'Inter',-apple-system,sans-serif",color:"var(--text)",maxWidth:"520px",margin:"0 auto",padding:"0"}}>
      ...
      <div style={{ padding:"28px 20px 20px",background:"linear-gradient(180deg,#12121f 0%,#0a0a0f 100%)",borderBottom:"1px solid var(--border)" }}>
        ...
        <div style={{ display:"flex",alignItems:"center",gap:"8px",marginBottom:"16px" }}>
          ...
          <span style={{ fontSize:"11px",color:"var(--muted)" }}>{sport === "mlb" ? "Baseball" : sport === "nba" ? "Basketball" : "College Basketball"}</span>
      ...
      <div style={{ margin:"0 12px",padding:"12px 14px",borderRadius:"10px",background:"var(--pred-bg)",border:"1px solid var(--pred-border)" }}>
        <div style={{ fontSize:"10px",fontWeight:700,color:"var(--accent)",textTransform:"uppercase",letterSpacing:"1px",marginBottom:"6px" }}>{sport === "mlb" ? "MLB Prediction Engine" : sport === "nba" ? "NBA Prediction Engine" : "Consensus Model v4"}</div>
