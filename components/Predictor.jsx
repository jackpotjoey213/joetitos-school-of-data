...
async function fetchLiveUpdates(games, lockedPredictions) {
...
// --- PREDICTION LOCKING (persists to localStorage) ---
function getLockedPredictions() {
  try {
    return JSON.parse(localStorage.getItem("joetito_locked_predictions") || "{}");
  } catch { return {}; }
}
function lockPrediction(gameId, prediction) {
  const locked = getLockedPredictions();
  if (!locked[gameId]) { // Only lock once - never overwrite
    locked[gameId] = { ...prediction, lockedAt: new Date().toISOString() };
    localStorage.setItem("joetito_locked_predictions", JSON.stringify(locked));
  }
  return locked;
}
function getAllLockedPredictions() {
  return getLockedPredictions();
}
        ...
        {liveAdj?.consensus ? (
          <div style={{ background:"#10b98110",border:"1px solid #10b98130",borderRadius:"10px",padding:"12px" }}>
            <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"8px" }}>
              <span style={{ fontSize:"10px",fontWeight:700,color:"#10b981",textTransform:"uppercase",letterSpacing:"1px" }}>KenPom Prediction</span>
            ...
            {liveAdj.consensus.modelTotal > 0 && (
              <div style={{ display:"flex",alignItems:"center",justifyContent:"center",gap:"12px",marginBottom:"10px" }}>
                <div style={{ textAlign:"center" }}>
                  ...
                  <span style={{ fontSize:"22px",fontWeight:800,fontFamily:"'JetBrains Mono',monospace",color:"var(--text)" }}>{liveAdj.consensus.awayKenPom ? Math.round((liveAdj.consensus.modelTotal - liveAdj.consensus.modelSpread) / 2) : "?"}</span>
                </div>
                ...
                <div style={{ textAlign:"center" }}>
                  ...
                  <span style={{ fontSize:"22px",fontWeight:800,fontFamily:"'JetBrains Mono',monospace",color:"var(--text)" }}>{liveAdj.consensus.homeKenPom ? Math.round((liveAdj.consensus.modelTotal + liveAdj.consensus.modelSpread) / 2) : "?"}</span>
                </div>
              </div>
            )}
            {/* Main calls */}
            <div style={{ display:"flex",gap:"6px",flexWrap:"wrap",justifyContent:"center",marginBottom:"10px" }}>
              ...
              {liveAdj.consensus.modelTotal > 0 && game.total > 0 && Math.abs(liveAdj.consensus.modelTotal - game.total) >= 3 && (
                <span style={{ background:"#f59e0b18",color:"#f59e0b",border:"1px solid #f59e0b40",padding:"3px 8px",borderRadius:"6px",fontSize:"10px",fontWeight:700 }}>3+ PT EDGE</span>
              )}
            </div>
            {/* Key data points */}
            <div style={{ display:"flex",gap:"8px",justifyContent:"center",marginBottom:"8px",fontSize:"10px",flexWrap:"wrap" }}>
              ...
              {liveAdj.consensus.liveProjectedTotal && <span style={{ background:"#ef444420",padding:"3px 8px",borderRadius:"4px",color:"#ef4444",fontWeight:700 }}>Live: {liveAdj.consensus.liveProjectedTotal}</span>}
            </div>
            {/* Reasoning */}
...
export default function App() {
  ...
  const handleRefresh = useCallback(async()=>{
    ...
    try {
      const currentLocked = getAllLockedPredictions();
      const result = await fetchLiveUpdates(games, currentLocked);
      ...
      if(updates&&Array.isArray(updates)){
        ...
        const ug=games.map(g=>{
          ...
          // LOCK new predictions immediately so they can't change later
          ...
          if(u.status==="final"&&u.awayScore!=null&&u.homeScore!=null){
            const lockedPred = getLockedPredictions()[g.id];
            if(lockedPred && !history.find(h=>h.gameId===g.id)){
              ...
              const predTotal = (lockedPred.predAway || 0) + (lockedPred.predHome || 0);
              const rec={gameId:g.id,away:g.awayAbbr,home:g.homeAbbr,predictedTotal:predTotal||0,actualTotal:at,totalCall:lockedPred.totalCall,line:lockedPred.line,totalCorrect:tOk,spreadCorrect:false,totalError:at-(predTotal||at),date:new Date().toISOString().split("T")[0]};
    ...
    <div style={{"--bg":"#0a0a0f","--card-bg":"#12121a","--card-header":"#0e0e16","--border":"#1e1e2e","--text":"#e8e8f0","--text-secondary":"#a0a0b8","--muted":"#5a5a78","--accent":"#6366f1","--pred-bg":"#6366f108","--pred-border":"#6366f120","--lines-bg":"#0a0a12","--pill-bg":"#1a1a28","--pill-active":"#6366f1",minHeight:"100vh",background:"var(--bg)",fontFamily:"'Inter',-apple-system,sans-serif",color:"var(--text)",maxWidth:"520px",margin:"0 auto",padding:"0"}}>
      ...
      <div style={{ margin:"0 12px",padding:"12px 14px",borderRadius:"10px",background:"var(--pred-bg)",border:"1px solid var(--pred-border)" }}>
        ...
        <div style={{ fontSize:"11px",color:"var(--text-secondary)",lineHeight:1.6 }}>
          <strong style={{color:"var(--text)"}}>3 data sources</strong> - The Odds API (live lines from 15+ sportsbooks), ESPN (live scores), and the KenPom math model drive predictions. Gemini AI provides supplementary analysis. <strong style={{color:"var(--text)"}}>Consensus picks</strong> are strongest when all sources agree. Hit <strong style={{color:"var(--accent)"}}>Refresh</strong> to pull all sources simultaneously.
