      ...
      <div style={{ padding:"16px" }}>
        ...
        {liveAdj?.consensus && (
          <div style={{ background:"#10b98110",border:"1px solid #10b98130",borderRadius:"10px",padding:"12px",marginTop:"10px" }}>
            <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"8px" }}>
              <span style={{ fontSize:"10px",fontWeight:700,color:"#10b981",textTransform:"uppercase",letterSpacing:"1px" }}>🤝 Consensus ({liveAdj.consensus.votes?.over + liveAdj.consensus.votes?.under} votes)</span>
              <span style={{ fontSize:"11px",fontWeight:800,color:liveAdj.consensus.strength>=75?"#22c55e":"#eab308",fontFamily:"'JetBrains Mono',monospace" }}>{liveAdj.consensus.strength}%</span>
            </div>
            <div style={{ display:"flex",gap:"6px",flexWrap:"wrap",justifyContent:"center",marginBottom:"8px" }}>
              <CallBadge text={`${liveAdj.consensus.totalCall} ${game.total} (${liveAdj.consensus.votes?.over}v${liveAdj.consensus.votes?.under})`} type={liveAdj.consensus.totalCall?.includes("OVER")?"over":"under"} />
            ...
            {liveAdj.claudePred && (
              <div style={{ fontSize:"10px",color:"var(--text-secondary)",lineHeight:1.6,borderTop:"1px solid #10b98120",paddingTop:"6px" }}>
                <div>🟣 <strong>Claude:</strong> {liveAdj.claudePred.call} — {liveAdj.claudePred.reason || liveAdj.claudePred.keyFactor || ""}</div>
                <div>🔵 <strong>Gemini:</strong> {liveAdj.geminiPred?.call} — {liveAdj.geminiPred?.reason || liveAdj.geminiPred?.keyFactor || ""}</div>
                {liveAdj.numBooks > 0 && <div>📊 <strong>Odds API:</strong> {liveAdj.numBooks} books, range {liveAdj.oddsRange?.min}-{liveAdj.oddsRange?.max}</div>}
              </div>
            )}
            {liveAdj.consensus.reasons?.length > 0 && (
              <div style={{ marginTop:"6px",fontSize:"10px",color:"#10b981",lineHeight:1.5 }}>
                {liveAdj.consensus.reasons.map((r,i) => <div key={i}>→ {r}</div>)}
              </div>
            )}
          </div>
        )}
  ...
  const handleRefresh = useCallback(async()=>{
    ...
    try {
      ...
      if(updates&&Array.isArray(updates)){
        ...
        const srcList = sources ? Object.entries(sources).map(([k,v])=>`${k}: ${v.startsWith("OK")?"✅":"❌"}`).join(" | ") : "";
