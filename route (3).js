...
// Allow up to 30 seconds for this route (Vercel hobby plan max is 60s)
...
async function askGemini(prompt) {
  ...
  try {
    ...
    if (data.error) {
      return { error: `${model}: ${data.error.message}` };
    }
...
export async function POST(req) {
  ...
  // FIX 1: ONLY analyze SCHEDULED games — never live or final
  // Once a game tips off, its prediction is frozen forever
  const gamesToAnalyze = gameSummaries.filter(g => {
    const status = g.espnScore?.status || g.status;
    if (status === "final") return false;
    if (status === "live") return false;  // NEVER re-predict live games
    if (locked[g.id]) return false;       // Already have a locked prediction
    return true;
  });
  ...
  const results = gameSummaries.map(g => {
    ...
    if (status === "final" && locked[g.id]) {
      return {
        ...
        awayScoreAdj: 0, homeScoreAdj: 0, injuries: null, trend: null, lineMove: null
      };
    }

    // If game is final but no locked prediction, just return scores honestly
    if (status === "final") {
      return {
        id: g.id, status: "final",
        awayScore: espn?.awayScore ?? null, homeScore: espn?.homeScore ?? null,
        ...
        currentSpread: null, oddsRange: null, numBooks: 0,
        oddsSpread: null, oddsTotal: null, oddsML: null,
        geminiPred: null, consensus: null, lockedPrediction: null,
        noPregamePrediction: true,
        isNewPrediction: false,
        awayScoreAdj: 0, homeScoreAdj: 0, injuries: null, trend: null, lineMove: null
      };
    }

    // FIX 1: LIVE games — return locked prediction + live score, but NEVER re-predict
    if (status === "live") {
      return {
        id: g.id, status: "live",
        awayScore: espn?.awayScore ?? null, homeScore: espn?.homeScore ?? null,
        clock: espn?.detail || espn?.clock || "",
        currentTotal: odds?.consensusTotal || locked[g.id]?.line || g.total,
        ...
        geminiPred: locked[g.id]?.geminiPred || null,
        consensus: locked[g.id]?.consensus || null,
        lockedPrediction: locked[g.id] || null,
        noPregamePrediction: !locked[g.id],
        isNewPrediction: false,
        awayScoreAdj: 0, homeScoreAdj: 0, injuries: null,
        trend: null, lineMove: null
      };
    }
    ...
    // FIX 2: HONEST CONFIDENCE CALCULATION
    // Factor 1: Agreement (how lopsided the votes are) - 30% of score
    const agreement = totalWeight > 0 ? (Math.max(overWeight, underWeight) / totalWeight) : 0.5;
    
    // Factor 2: Coverage (how many signals voted) - 30% of score
    const signalCount = reasons.filter(r => !r.startsWith("Odds API:")).length; // Don't count Odds API info line
    const coverageScore = Math.min(1, signalCount / 3); // 3+ voting signals = full coverage
    
    // Factor 3: Edge reasonableness - 20% of score
    // 2-5 pt edge = best (model likely right)
    // 5-8 pt edge = good
    // 8-12 pt edge = suspicious
    // 12+ pt edge = model probably wrong, penalize heavily
    const edgeSize = modelTotal > 0 && currentTotal > 0 ? Math.abs(modelTotal - currentTotal) : 0;
    let edgeScore = 0.5; // default
    if (edgeSize >= 2 && edgeSize <= 5) edgeScore = 1.0;      // sweet spot
    else if (edgeSize > 5 && edgeSize <= 8) edgeScore = 0.85;  // solid
    else if (edgeSize > 8 && edgeSize <= 12) edgeScore = 0.6;  // suspicious
    else if (edgeSize > 12) edgeScore = 0.35;                   // model probably wrong
    else if (edgeSize < 2) edgeScore = 0.4;                     // too close to call
    
    // Factor 4: Line tightness (sharp line = harder to beat) - 20% of score
    let lineTightness = 0.5; // default
    if (odds?.totalRange) {
      const spread = (odds.totalRange.max || 0) - (odds.totalRange.min || 0);
      if (spread <= 1 && edgeSize > 5) lineTightness = 0.3;    // books agree tightly, big divergence = risky
      else if (spread <= 1) lineTightness = 0.7;                // books agree, small edge = decent
      else if (spread >= 3) lineTightness = 0.8;                // books disagree = more opportunity
      else lineTightness = 0.5;
    }
    
    // Final confidence: weighted combination, capped 25-95%
    const rawConfidence = (agreement * 30) + (coverageScore * 30) + (edgeScore * 20) + (lineTightness * 20);
    const consensusStrength = Math.min(95, Math.max(25, Math.round(rawConfidence)));
